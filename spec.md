...
* **交互方式**：长按麦克风按钮开始录音，松开结束录音并发送。
* **实现路径**：
1. 优先调用浏览器原生的 `Web Speech API`，前端实时转写，完成录音后将文本通过 WebSocket 发送。
2. 降级方案：前端通过 `MediaRecorder` 采集 `WebM/WAV` 音频流，POST 给后端，后端调用 Whisper 模型转文本。


* **自动换行机制**：语音文本发送时，系统需自动在末尾追加 `\r`（回车符），触发终端执行。
