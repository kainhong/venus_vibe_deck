...
CLI 配置通过 `server/config/settings.json` 维护。每条 CLI 配置包含：
...
打开 **更多 → 设置** 可以调整显示偏好并查看语音配置。

- **显示 / 操作习惯**：选择右手或左手模式。左手模式会镜像底部操作栏，让回车靠左。
- **语音**：显示服务端语音是否开启，并列出已配置的语音命令别名。

运行时 CLI 和语音命令变更请编辑 `server/config/settings.json`。
