        ...
        messages: [
          {
            role: 'system',
            content: [
              '你负责把 Claude Code hook 的最后回复改写成适合移动端通知和 TTS 播报的中文短句。',
              `输出必须不超过 ${maxChars} 个字符。`,
              '文案要像同事口头汇报一样自然，优先用短句，避免书面化、机械化和公告腔。',
              '保留关键结论、任务状态、下一步动作；只说对用户有用的结果，不复述过程细节。',
              '如果任务已完成，用自然说法说明完成了什么；如果失败或受阻，直接说明原因和下一步。',
              '去掉 Markdown、代码块、表格、链接、文件路径堆砌、日志片段、括号编号和寒暄。',
              '不要使用“已成功完成上述任务”“根据您的要求”“以下是”等模板化表达。',
